...
% v0.11, 23.3.2015
% 
% Note: you need wav-files for the challenge subset of Buckeye corpus.
...
folder = '/Users/orasanen/speechdb/zerospeech/english_wavs/';       % Location of audio files

savefile = 'patterns_english.txt';
...
% Get talker IDs for each signal (a total of 68 signals from 12 unique
% talkers). Variable "talkers" keeps track which talker speaks which of the
% 68 signals.
talker_names = cell(length(signal_filenames),1);
for k = 1:length(signal_filenames)
    talker_names{k} = signal_filenames{k}(end-8:end-6);
end
unique_talkers = unique(talker_names);
talkers = zeros(length(signal_filenames),1);
for k = 1:length(signal_filenames)
    talkers(k) = find(strcmp(unique_talkers,signal_filenames{k}(end-8:end-6)));
end
...
if(strcmp(segment_algorithm,'oscillator') && verbose)
    drawOscillatorSyllableSegExample(original_audio,bounds,envelopes,osc_env);
elseif(verbose)
    drawSyllableSegExample(original_audio,bounds,envelopes);
end
...
vadmasks_100hz = runVAD(F);     % Get non-speech and speech regions with a VAD
...
[times_eva,classes_eva,signal_names_eva] = croptointervals(times,classes,signal_names);
...
writeResultOutput(savefile,signal_names_eva,times_eva,classes_eva);
